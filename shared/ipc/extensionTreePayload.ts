...
// 칸이 계속 느는 자리라(`detail`·`section`·`actions` 가 뒤에 붙었다) 앞으로도 자란다.
...
export interface ExtensionTreeNodePayload {
  ...
  /**
   * 줄 버튼이 **둘 이상**일 때. `action` 과 같은 것이되 개수 제한이 없다.
   *
   * 하나로는 못 담는 짝이 실제로 나왔다: 이미 쓴 시나리오는 **보고** 나서 **다시**
   * 만든다. 하나만 둘 수 있으면 둘 중 하나가 딴 데로 가는데, 어느 쪽을 보내도 그 줄에서
   * 하려던 일이 두 자리로 갈린다.
   *
   * `action` 을 그대로 둔 이유: 버튼이 하나인 확장이 대부분이고, 그쪽에 배열을 쓰게 하면
   * 이미 도는 확장을 전부 고쳐야 한다. 둘 다 오면 **이어서** 그린다.
   *
   * 폭이 300px 이라 셋을 넘기면 이름이 먹힌다 — 그때는 줄이 아니라 `⋯` 로 갈 때다.
   */
