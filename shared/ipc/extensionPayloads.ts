...
export interface ExtensionRunCommandPayload {
   ...
   *
   * **확장 화면에서 확장으로 가는 메시지 통로를 열지 않고 선택을 전하는 길이다.**
   * 그 통로는 파일 열기 하나만 통과시키고(`extensionHtmlDoc.ts` 의 `isOpenRequest`),
   * 넓히면 확장이 앱에 보낼 수 있는 것이 통째로 열린다. 대신 **조작 알갱이를 명령 단위로**
   * 두고, 명령을 걸 때 지금 고른 것을 함께 싣는다.
   *
