...
// `checkModels` 는 **두 겹**이다: 프로바이더·모델이 잡혀 있나 → 그 주소가 살아 있나.
//
// ⚠️ **경로가 두 세대로 갈린다** (README 실측 함정 1·8): health 는 `/global` 판을 쓰고
// providers 는 `/api` 판이 **없어** 레거시 표면(`/config/providers`)을 쓴다.
// 한쪽에 맞춰 통일하면 404 이고, 화면에는 원인이 아니라 "HTTP 404" 로 뜬다.
//
...
describe('checkModels — 프로바이더 주소가 살아 있나 (두 번째 겹)', () => {
  ...
  it('baseURL 을 그대로 ping 한다', async () => {
    ...
    expect(urlOf(impl, 1)).toBe('http://127.0.0.1:11434/v1')
  })
  ...
  it('baseURL 이 없는 프로바이더는 ping 을 건너뛴다', async () => {
    ...
    const result = await checkModels('http://127.0.0.1:4096', impl)
    expect(result.ok).toBe(true)
    expect(callCount(impl)).toBe(1)
  })

  // ⚠️ **「하나만 살아 있으면 통과」로 하면 원래 잡으려던 실패를 놓친다.**
  // 실측 설정이 정확히 이 모양이다 — 원격 `opencode` 가 항상 초록이라 ollama 가 죽어도
  // 그 규칙이면 초록이 된다. 그래서 baseURL 이 있는 것은 **전부** 살아 있어야 한다.
  it('원격 프로바이더가 같이 있어도 죽은 로컬 하나면 실패다', async () => {
    const impl = routedFetch(
      {
        providers: [
