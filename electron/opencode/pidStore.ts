...
export class ServerPidStore {
   ...
   *
   * **모르면 false 다.** 그런데 그 `false` 는 **네 가지를 한 값으로 뭉갠다**:
   *
   *   1. 기록이 없다 (우리가 안 띄웠다)
   *   2. 프로세스가 죽었다
   *   3. 그 번호를 남이 물려받았다
   *   4. **잴 수 없었다** — `ps` fork 가 실패했거나(`commandOf` → null),
   *      흔적 파일을 못 써서(`write` 는 실패를 삼킨다) 목록이 비었다
   *
   * 앞 셋은 뜻이 같지만 **4번은 다르다.** 살아 있는 우리 서버가 조용히 「남의 것」이 된다.
   * 실측에서 그 모양이 한 번 관측됐고 재현은 안 됐다 (2026-08-16, contract-qa).
   *
   * ⚠️ **지금 이게 견딜 만한 이유는 `ours` 가 문구만 가르기 때문이다** — 조치는 주인이
   * 누구든 `restart` 하나라(설계 §1) 4번이 터져도 대가는 틀린 안내 한 줄이다.
   * **`ours` 로 조치나 프로세스 조작을 가르게 되는 날, 이 네 갈래를 먼저 갈라야 한다.**
   * 안 가르면 "잴 수 없었다" 가 "죽여도 된다/안 된다" 를 정하게 된다.
   */
