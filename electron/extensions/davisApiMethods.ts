// 확장이 부를 수 있는 **메서드 이름들**. `davis` 객체(`davisApi.ts`)의 각 함수가 이 중
// 하나로 부모에게 넘어가고, `serviceDispatch.ts` 가 같은 이름으로 받아 갈래를 탄다.
//
// `davisApi.ts` 에서 갈라냈다 — 저쪽이 300줄 상한에 닿았다. 이름과 그 뜻은 **확장 개발자가
// 읽는 계약**이라 주석이 길고, 앞으로도 기여점이 늘 때마다 길어진다. 갈래 실행(대리자)과
// 이름 목록은 같은 속도로 자라지 않는다. `davisApi.ts` 가 그대로 다시 내보내므로
 ...
 *
 * **확장에 라이선스 키를 주지 않는다.** 호스트가 자기 소켓으로 대신 묻고 답만 돌려준다
 * (`electron/agentLane/askAgent.ts`). 확장이 직접 소켓을 열게 하면 설치된 모든 확장이
 * 키를 갖게 되고, 배포처로 남의 확장을 받는 순간 그대로 위험이 된다.
 *
 * 그 레인은 **읽기 전용**이다 (`set_permission_mode { mode: 'plan' }`). 확장이 시킨
 * 분석이 사용자의 파일을 고치는 일은 없다.
 */
export const METHOD_AGENT_ASK = 'agent.ask'
/**
 ...
 *
 * 확장은 자기 이름을 **말하지 않는다** — 호스트가 어느 확장에 준 `davis` 인지 알고
 * 대신 채운다 (`createDavisApi` 의 두 번째 인자). 확장이 이름을 실어 보내면 남의 칸을
 * 읽는 것을 막을 방법이 없다.
 */
