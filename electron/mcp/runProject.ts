...
// 같은가" 를 본다.
//
// ⚠️ **무엇을 실행할지는 이번엔 인자로 받는다.** AGENTS.md 의 「실행」 절에서 읽는 길
// (설계 §2)은 아직 없다 — 그 자리를 여기서 지어내지 않는다. 모델이 명령 문자열을 준다.
...
export function runProjectInput(args: Record<string, unknown>): RunProjectInput {
  ...
  // 제어문자는 `open_terminal` 과 같은 이유로 막는다(실측 근거는 그쪽 머리말). 다만 뜻이
  // 다르다: 여기서는 개행이 **명령을 여럿으로 쪼개고**, 그러면 우리가 "이것을 띄웠다" 고
  // 돌려준 문장과 실제로 돈 것이 달라진다.
