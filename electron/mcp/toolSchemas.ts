...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      '이 프로젝트를 **어떻게 띄우는지**를 알아낸 뒤 그 목록을 앱에 적어 둔다. 사용자가 사이드바 「실행」 패널에서 ▶ 로 누르는 목록이 그것이고, 한 번 적어 두면 다음부터는 모델을 부르지 않고 앱이 기억해 둔 것을 읽는다. 사용자가 "실행 방법을 찾아 달라" 고 할 때, 매니페스트가 바뀌어 "다시 확인" 을 청할 때 쓴다. **먼저 프로젝트를 읽어라** — package.json 의 scripts, gradle 태스크, Makefile, docker-compose, README 를 보고 정하고, 규칙으로 짐작하지 마라(모노레포·워크스페이스·`dev` 가 아니라 `start:local` 인 프로젝트가 실제로 있다). 이 도구는 목록을 적기만 하고 **아무것도 실행하지 않는다** — 돌리는 것은 run_project 다. 목록은 앱이 프로젝트 밖에 들고 있어 **이 도구 말고는 적을 길이 없다**: 프로젝트 파일(AGENTS.md 등)에 적어 두면 패널은 그것을 영영 못 읽는다.',
    inputSchema: {
      type: 'object',
      properties: {
        commands: {
