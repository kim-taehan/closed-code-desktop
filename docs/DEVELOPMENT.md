...
>
> **2026-08-28 정정 — 모델도 한도도 바뀌었다** (사내 공지 + 실측).
> `glm-5.2` 는 **종료됐다.** 지금 사내에 서는 것은 `qwen3.8-27b`(기본) · `qwen3.5-122b` ·
> `qwen3.6-35b` 셋이고, 우리는 앞의 둘만 등록했다. 위 「검증용 / 약한 모델 판정용」 구분은
> 그래서 없어졌다 — **약한 모델을 재던 자리는 비어 있다.**
>
> **폴백이 없어졌고, 한도는 262K 다.** 위 문단이 적은 「128K 를 넘으면 외부 제공자로 폴백」은
> 더 이상 사실이 아니다. 지금은 **262K 를 넘기면 400 으로 즉시 실패한다** — 조용히 외부로
> 넘기면 성공한 것처럼 보이면서 프롬프트가 사외로 나가기 때문에 일부러 그렇게 뒀다는 것이
> 사내 설명이다. **이 문단이 기록해 둔 이분 탐색 결과(130,015 / 132,013)는 그때의 사실이고,
> 지금 그 경계는 없다.**
>
> 그리고 **사내 게이트웨이를 지나는 프롬프트·응답은 관측용으로 적재된다** —
> 고객 데이터·개인정보를 넣지 않는다.
