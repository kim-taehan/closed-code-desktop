...
describe('belongsToApp', () => {
  ...
  describe('xterm ctrl 표 — 셸이 바이트를 받는 것과 못 받는 것', () => {
    ...
    // ⚠️ **이 둘은 위 표에 없다.** shift 가 섞이는 **다른 갈래**에서 바이트가 나온다
    // (`⌃⇧-` → US(readline undo), `⌃⇧2` → NUL(set-mark)). `DrawerKeyLike` 가 shiftKey 를
    // 안 받아 여기서는 한 글자 `_`·`@` 로만 보이므로, 제외에 안 넣으면 조용히 앱으로 샌다.
    // 앱에는 그 단축키가 없어 **아무 일도 안 일어나고** "가끔 undo 가 안 먹는다" 로만 겪힌다.
    it('_ 와 @ 도 셸 것이다 — 근거가 다른 갈래에 있다', () => {
      shellGets('_')
      shellGets('@')
    })
