 ...
 *
 * `idle` 에서 열면 **첫 렌더마다 팝업이 뜬다.** 시도조차 안 한 상태이고, 프로젝트를
 * 활성화하면 지연 연결이 곧바로 도니 서버가 없으면 1~2초 안에 `error` 가 된다.
 * `disconnected` 에서 안 여는 것도 같은 결이다 — 재연결 중에 튀어나오는 팝업은 방해다.
 *
 ...
 *
 * ⚠️ **상태가 `error` 를 벗어났다고 닫지 않는다.** 치유(재연결)가 도는 동안 상태는
 * `error → connecting → error` 로 오갈 수 있는데, 그때 닫아 버리면 이미 "한 번 열었다" 로
 * 표시돼 **다시 안 열린다.** 닫는 것은 사용자(`close`)와 진단 성공(`onHealthy`) 둘뿐이다.
 */
export function useDoctorGate(projectId: string | undefined, status: ProjectStatus | undefined): DoctorGate {
